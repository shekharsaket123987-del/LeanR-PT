...
3. **Cancellation session-credit refund**: does cancelling a session restore a session to the subscription's remaining count? Not evident in `cancelSessionAction`'s direct code path — may happen via a DB trigger/function not read in this pass. Needs confirmation against `bookings.service.ts`/relevant migrations before the mobile app assumes either behavior.
