        ...
        {escalations.some((e) => e.status === "open") && (
          <Card className="space-y-2 p-5">
            ...
            {escalations
              ...
              .map((e) => (
                <div key={e.id} className="rounded-xl border border-red-400/30 bg-red-400/10 p-3">
                  <p className="text-sm font-semibold text-red-300">{e.reason}</p>
                  {e.description && <p className="mt-0.5 text-xs text-red-300/80">{e.description}</p>}
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="flex items-center gap-3">
                <button
                  type="button"
                  className="glass-faint rounded-lg p-2"
                  onClick={() => setSessionDelta((d) => d - 1)}
                >
                  <MinusCircle className="h-4 w-4" />
                </button>
                <input readOnly value={sessionDelta} className="w-20 glass-faint rounded-xl p-2 text-center text-sm text-white" />
                <button
                  type="button"
                  className="glass-faint rounded-lg p-2"
                  onClick={() => setSessionDelta((d) => d + 1)}
        ...
        {client.subscription && (
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="flex items-center gap-3">
                <button type="button" className="glass-faint rounded-lg p-2" onClick={() => setPauseDaysDelta((d) => d - 1)}>
                  <MinusCircle className="h-4 w-4" />
                </button>
                <input readOnly value={pauseDaysDelta} className="w-20 glass-faint rounded-xl p-2 text-center text-sm text-white" />
                <button type="button" className="glass-faint rounded-lg p-2" onClick={() => setPauseDaysDelta((d) => d + 1)}>
                  <PlusCircle className="h-4 w-4" />
                </button>
              </div>
      ...
      <Modal open={modal === "transfer"} onClose={closeModal} title="Transfer to Another Coach">
        <div className="space-y-4">
          ...
          <select
            ...
            className="w-full glass-faint rounded-xl p-3 text-sm text-white outline-none focus:border-brand-yellow focus:ring-1 focus:ring-brand-yellow"
          >
          ...
          {transferAvailabilityWarning ? (
            <div className="space-y-3 rounded-xl border border-amber-400/30 bg-amber-400/10 p-3">
              <p className="text-xs text-amber-300">{transferAvailabilityWarning}</p>
            ...
            {(
              ...
              <div key={key}>
                ...
                <input
                  ...
                  className="w-full glass-faint rounded-xl p-2.5 text-sm text-white"
                />
              </div>
            ))}
          </div>
