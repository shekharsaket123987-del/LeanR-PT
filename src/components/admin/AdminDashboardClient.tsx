    ...
    <>
      ...
      <div className="mt-8 grid grid-cols-1 gap-6 lg:grid-cols-3">
        <Card className="p-6 lg:col-span-2">
          ...
          <p className="mb-4 text-xs text-white/45">Last 6 months</p>
          <ResponsiveContainer width="100%" height={260}>
            <LineChart data={revenueTrend}>
              <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#FFFFFF14" />
              <XAxis dataKey="month" tickLine={false} axisLine={false} tick={{ fontSize: 12, fill: "#FFFFFF66" }} />
              <YAxis
                tickLine={false}
                axisLine={false}
              ...
              />
              <Tooltip
                contentStyle={{ borderRadius: 12, border: "1px solid #FFFFFF1A", background: "#0c0c0c", color: "#fff", fontSize: 12 }}
                formatter={(v: number) => [`₹${(v / 100000).toFixed(1)}L`, "Revenue"]}
              />
              <Line type="monotone" dataKey="revenue" stroke="#F5D90A" strokeWidth={2.5} dot={{ fill: "#F5D90A", stroke: "#0c0c0c", strokeWidth: 2, r: 4 }} />
            </LineChart>
          </ResponsiveContainer>
        </Card>
        ...
        <Card className="p-6">
          ...
          <p className="mb-4 text-xs text-white/45">Across active clients</p>
            ...
            {utilizationByCoach.slice(0, 5).map((c) => (
              <div key={c.name}>
                <div className="mb-1 flex justify-between text-xs">
                  ...
                  <span className="text-white/40">{c.utilization}%</span>
                </div>
      ...
      <Card className="mt-6 p-6">
        ...
        <p className="mb-4 text-xs text-white/45">Today across all coaches</p>
        <ResponsiveContainer width="100%" height={220}>
          <BarChart data={bookingsByHour}>
            <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#FFFFFF14" />
            <XAxis dataKey="hour" tickLine={false} axisLine={false} tick={{ fontSize: 12, fill: "#FFFFFF66" }} />
            <YAxis tickLine={false} axisLine={false} tick={{ fontSize: 12, fill: "#FFFFFF66" }} width={24} />
            <Tooltip cursor={{ fill: "#FFFFFF0A" }} contentStyle={{ borderRadius: 12, border: "1px solid #FFFFFF1A", background: "#0c0c0c", color: "#fff", fontSize: 12 }} />
            <Bar dataKey="bookings" fill="#F5D90A" radius={[6, 6, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </Card>
    </>
