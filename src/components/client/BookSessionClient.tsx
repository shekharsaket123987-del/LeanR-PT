...
import { Sparkles, CalendarDays, Check, ChevronRight, CalendarPlus, AlertCircle, Scale } from "lucide-react";
...
export default function BookSessionClient({ options, measurementsStale }: { options: BookingOptions; measurementsStale: boolean }) {
    ...
    <div className="mx-auto max-w-3xl">
      ...
      {measurementsStale && (
        <div className="mb-6 flex items-start gap-3 rounded-xl border border-red-500/30 bg-red-500/5 p-4">
          <Scale className="mt-0.5 h-4 w-4 shrink-0 text-red-400" />
          <div>
            <p className="text-sm font-bold text-red-400">Update your measurements to book a session</p>
            <p className="mt-0.5 text-xs text-red-400/80">
              You can pick a time below, but booking won&apos;t go through until you&apos;ve logged this week&apos;s measurements.{" "}
      ...
      <div className="mb-8 flex items-center gap-2">
        {["intro", "schedule", "confirm"].map((s, i) => (
          <div key={s} className="flex flex-1 items-center gap-2">
            ...
            {i < 2 && <div className="h-0.5 flex-1 bg-white/10" />}
          </div>
        ))}
      </div>
      ...
      {step === "intro" && !isFirstSession && (
        <Card className="p-6">
          <p className="mb-4 text-xs font-bold uppercase tracking-wide text-white/40">Your Coach</p>
          ...
          <p className="mt-4 rounded-xl bg-white/[0.03] p-3 text-xs text-white/50">
            You&apos;re booking with your assigned coach. Coach continuity is core to your plan — need a change?{" "}
            <a href="/client/coach" className="font-bold text-white underline">
      ...
      {step === "schedule" && (
        <Card className="p-6">
          <p className="mb-1 text-xs font-bold uppercase tracking-wide text-white/40">Pick a time</p>
          <p className="mb-5 text-sm text-white/50">Showing {coach.name.split(" ")[0]}&apos;s next open slots.</p>
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            {slots.map((slot) => (
              ...
              >
                ...
                <p className="text-xs text-white/45">{formatTime(slot.start)} · {durationMinutes} min</p>
              </button>
            ))}
          </div>
      ...
      {step === "confirm" && !bookedId && selectedSlot && (
        <Card className="p-6">
          <p className="mb-1 text-xs font-bold uppercase tracking-wide text-white/40">Review &amp; confirm</p>
          ...
          {confirmError && (
            <p className="mt-4 flex items-center gap-2 rounded-lg border border-red-500/30 bg-red-500/10 px-3 py-2 text-xs text-red-400">
          ...
          <div className="mt-7 flex justify-between">
            ...
            <Button onClick={confirmBooking} loading={confirming} disabled={measurementsStale}>
              Confirm Booking
            </Button>
          </div>
        </Card>
