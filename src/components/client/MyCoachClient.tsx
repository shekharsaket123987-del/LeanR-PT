    ...
    <div className="flex gap-1">
      {[1, 2, 3, 4, 5].map((n) => (
        <button key={n} type="button" onClick={() => onChange(n)}>
          <Star className={`h-5 w-5 ${n <= value ? "fill-brand-yellow text-brand-yellow" : "text-white/20"}`} />
        </button>
      ))}
    </div>
  );
    ...
    return (
      <Card className="overflow-hidden">
        ...
        <div className="relative -mt-14 px-6">
          <div className="relative h-28 w-28 overflow-hidden rounded-2xl border-4 border-surface-elevated">
        ...
        <div className="p-6 pt-4">
          ...
          <p className="mt-1 text-sm font-medium text-white/50">{coach.specialization}</p>
  ...
  return (
    <>
      ...
      {request?.status === "rejected" && (
        <Card className="mb-6 border-red-400/30 bg-red-400/10 p-4">
          <p className="text-sm font-bold text-red-300">Request Rejected</p>
          <p className="mt-1 text-xs text-red-400">Our team reviewed your request and you'll continue with {coach.name} for now.</p>
        </Card>
      )}
      {request?.status === "approved" && !needsCompletion && (
        <Card className="mb-6 border-emerald-400/30 bg-emerald-400/10 p-4">
      ...
      {needsCompletion && (
        <Card className="mb-6 p-5">
          ...
          <p className="mt-1 text-xs text-white/45">Choose the days and time you'd like, and we'll find an available coach.</p>
      ...
      <Card className="overflow-hidden">
        ...
        <div className="p-6 pt-4">
          ...
          <p className="mt-1 text-sm font-medium text-white/50">{coach.specialization}</p>
          {coach.bio && <p className="mt-4 text-sm leading-relaxed text-white/65">{coach.bio}</p>}

          <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-3">
            <div className="rounded-xl bg-white/[0.04] p-4">
              <Award className="mb-2 h-4 w-4 text-white/50" />
              <p className="text-xs font-bold uppercase text-white/40">Certifications</p>
              <p className="mt-1 text-xs text-white/60">{coach.certifications.join(", ") || "—"}</p>
            </div>
            <div className="rounded-xl bg-white/[0.04] p-4">
              <Languages className="mb-2 h-4 w-4 text-white/50" />
              <p className="text-xs font-bold uppercase text-white/40">Languages</p>
              <p className="mt-1 text-xs text-white/60">{coach.languages.join(", ") || "—"}</p>
            </div>
            <div className="rounded-xl bg-white/[0.04] p-4">
              <Users className="mb-2 h-4 w-4 text-white/50" />
              <p className="text-xs font-bold uppercase text-white/40">Experience</p>
              <p className="mt-1 text-xs text-white/60">{coach.yearsExperience} years coaching</p>
            </div>
          </div>

          <div className="mt-8 border-t border-white/[0.06] pt-6">
            <p className="text-sm font-bold">Not the right fit?</p>
            <p className="mt-1 text-xs text-white/45">You can request a coach change. Our team reviews every request personally.</p>
        ...
        <div className="space-y-4">
          <div>
            ...
            <textarea
              ...
              className="glass-faint w-full rounded-xl p-3 text-sm text-white placeholder:text-white/30 outline-none focus:border-white/20"
            />
          </div>
          <div>
            <label className="mb-1.5 block text-xs font-bold uppercase text-white/40">Overall Experience</label>
            <StarPicker value={overallExperience} onChange={setOverallExperience} />
          </div>
          <div>
            <label className="mb-1.5 block text-xs font-bold uppercase text-white/40">Coach Rating</label>
            <StarPicker value={coachRating} onChange={setCoachRating} />
          </div>
          <div>
            <label className="mb-1.5 block text-xs font-bold uppercase text-white/40">Additional Comments</label>
            <textarea value={additionalComments} onChange={(e) => setAdditionalComments(e.target.value)} rows={2} className="glass-faint w-full rounded-xl p-3 text-sm text-white outline-none focus:border-white/20" />
          </div>
