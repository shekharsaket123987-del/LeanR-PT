...
export default function CoachProfileClient({ profile }: { profile: CoachProfileView }) {
  ...
  async function save() {
    setBusy(true);
    setError("");
    const result = await updateMyCoachProfileAction({ phone, emergencyContact, photoUrl: photoUrl || undefined });
    setBusy(false);
    if (isFailure(result)) {
    ...
    <>
      <Card className="p-6">
        ...
        <div className="mt-6 border-t border-white/[0.06] pt-6">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {profile.certifications.map((c) => (
              <span key={c} className="rounded-full bg-white/5 px-3 py-1 text-xs font-semibold">
                {c}
              </span>
            ))}
          </div>
        ...
        <div className="mt-6 border-t border-white/[0.06] pt-6">
          ...
          <div className="mb-3 flex flex-wrap gap-2">
            ...
            {skills.map((s) => (
              <span key={s} className="rounded-full bg-white/5 px-3 py-1 text-xs font-semibold">
                {s}
              </span>
            ))}
          </div>
        ...
        <div className="mt-6 grid grid-cols-2 gap-4 border-t border-white/[0.06] pt-6 sm:grid-cols-4">
          ...
          <div>
            ...
            <p className="text-sm">
              {profile.currentCapacity} / {profile.maxCapacity} ({profile.availableCapacity} open)
            </p>
          </div>
        </div>
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="Edit Contact Info">
        <div className="space-y-4">
          ...
          <div>
            <label className="mb-1.5 block text-xs font-bold uppercase text-white/40">Profile Picture</label>
            <div className="flex items-center gap-4">
