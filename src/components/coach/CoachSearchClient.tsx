    ...
    <div className="mx-auto max-w-2xl">
      <div className="relative">
        ...
        <input
          ...
          className="w-full rounded-xl border border-black/15 py-3 pl-10 pr-4 text-sm focus:border-brand-yellow focus:outline-none focus:ring-1 focus:ring-brand-yellow"
        />
      </div>

      <div className="mt-4 space-y-2">
        {query.trim().length === 0 && (
          <p className="px-1 text-xs text-black/40">Search across every client on the platform, not just your own.</p>
        )}
        ...
        {filtered.map((c) => (
          <Link key={c.id} href={`/coach/clients/${c.id}`}>
            <Card className="flex items-center gap-3 p-3.5 transition-colors hover:bg-black/[0.02]">
              <div className="relative h-10 w-10 shrink-0 overflow-hidden rounded-xl">
                <Image src={c.photo} alt={c.name} fill className="object-cover" />
              </div>
              <div className="min-w-0 flex-1">
                <p className="truncate text-sm font-bold">{c.name}</p>
                <p className="truncate text-xs text-black/45">{c.clientCode}</p>
              </div>
