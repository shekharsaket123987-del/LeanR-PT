        ...
        <div ref={ref} onMouseMove={handleMove} onMouseEnter={onHoverEnter} onMouseLeave={handleLeave} className="h-full [perspective:1000px]">
          ...
          >
            <GlassCard
              ...
              glow={isActive}
              className={cn(
                "group relative h-full cursor-pointer overflow-visible pt-10 transition-[border-color,box-shadow] duration-300",
                isActive && "border-brand-yellow/50"
              )}
            ...
            >
              ...
              <div className="px-5 pb-5 pt-1 text-center">
                <h3 className="text-display text-lg leading-tight text-white">{coach.name}</h3>
...
export default function Coaches() {
  ...
  function handleDragEnd(_: unknown, info: PanInfo) {
    const threshold = 60;
    if (info.offset.x < -threshold) setActiveIndex((i) => Math.min(i + 1, COACHES.length - 1));
    else if (info.offset.x > threshold) setActiveIndex((i) => Math.max(i - 1, 0));
  }
          ...
          <Reveal className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-end">
            <div>
              ...
              <h2 className="text-display mt-3 text-3xl sm:text-4xl md:text-5xl leading-[0.95]">
                THE <span className="text-brand-yellow italic-skew">COACHES</span>
              </h2>
            </div>
            <p className="max-w-sm text-sm text-white/60">
              Every LeanR coach is certified, background-verified, and trained specifically for live online
              delivery. Drag or tap to browse — tap the highlighted coach to read more.
            </p>
          </Reveal>
        ...
        {active && (
          <div>
            <div className="relative aspect-[16/9] overflow-hidden rounded-t-2xl bg-black">
              ...
              />
              <div className="absolute left-4 top-4 rounded-full bg-black/50 px-3 py-1 text-[10px] font-semibold uppercase tracking-wider text-brand-yellow backdrop-blur-md border border-brand-yellow/30">
                Certified
              </div>
            </div>
            <div className="p-6 sm:p-7">
              <h3 className="text-display text-2xl text-white">{active.name}</h3>
