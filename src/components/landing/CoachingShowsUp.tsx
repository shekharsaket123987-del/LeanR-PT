...
const CARDS: Card[] = [
  {
    ...
    to: "#0c0c0a",
    depth: "left",
    image: "/ChatGPT Image Aug 22, 2026, 02_18_13 PM.png",
  },
  {
    icon: "🏠",
    ...
    to: "#0a0a08",
    depth: "center",
    image: "/ChatGPT Image Aug 22, 2026, 03_57_24 PM.png",
  },
  {
    icon: "📈",
    ...
    to: "#0a0a08",
    depth: "right",
    image: "/ChatGPT Image Aug 22, 2026, 04_02_06 PM.png",
  },
];
...
function CoachCard({ card, index }: { card: Card; index: number }) {
  ...
  const rotateX = useSpring(useTransform(my, [-0.5, 0.5], [9, -9]), { stiffness: 220, damping: 20 });
  ...
  return (
    <div style={{ perspective: 1400 }} className="h-full">
      ...
      >
        <motion.div
          animate={{ y: [0, -8, 0] }}
          transition={{ duration: 4.5 + index * 0.5, repeat: Infinity, ease: "easeInOut", delay: index * 0.3 }}
          className="h-full"
        >
          <div ref={ref} onMouseMove={handleMove} onMouseLeave={handleLeave} className="h-full [perspective:1400px]">
            <motion.div style={{ rotateX, rotateY, transformStyle: "preserve-3d" }} className="h-full">
              <GlassCard
                variant={index === 1 ? "yellow" : "strong"}
                glow={index === 1}
                className="group relative flex h-full flex-col overflow-hidden rounded-[1.75rem] p-0"
              >
                <div
                ...
                <div className="flex flex-1 flex-col items-center p-5 pt-2 text-center">
                  <h3 className="text-display text-lg text-white">{card.title}</h3>
      ...
      <FlipSection className="container-px">
        <Reveal className="max-w-2xl">
          ...
          <h2 className="text-display mt-3 text-3xl sm:text-4xl md:text-5xl leading-[0.95]">
            COACHING
            <br />
            <span className="text-brand-yellow italic-skew">THAT SHOWS UP.</span>
          </h2>
        </Reveal>
