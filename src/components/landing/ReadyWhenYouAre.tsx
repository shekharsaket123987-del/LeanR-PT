...
function ShieldCheckIcon({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.6} className={className}>
      <path d="M12 2.5l7.5 3.2v5.6c0 4.6-3.2 7.8-7.5 9.7-4.3-1.9-7.5-5.1-7.5-9.7V5.7L12 2.5z" />
      <path strokeLinecap="round" strokeLinejoin="round" d="M8.5 12l2.4 2.4L15.5 10" />
    </svg>
  );
}
...
function CalendarIcon({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.6} className={className}>
      ...
      <path strokeLinecap="round" d="M8 14.5h2M14 14.5h2" />
    </svg>
  );
}
...
function HeadsetIcon({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.6} className={className}>
      <path strokeLinecap="round" d="M4 13v-2a8 8 0 0116 0v2" />
      <rect x="3" y="13" width="4" height="6" rx="1.5" />
      <rect x="17" y="13" width="4" height="6" rx="1.5" />
      <path strokeLinecap="round" d="M19 19v1a2 2 0 01-2 2h-3" />
    </svg>
  );
}
...
function MatIcon({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 64 40" className={className}>
      ...
      <text x="42" y="24" textAnchor="middle" fontSize="8" fill="#f5d90a" fontWeight="700">
        LEANR
      </text>
    </svg>
  );
...
function CoachPhone() {
  ...
  return (
    ...
    >
      <motion.div
        animate={
          floating ? { y: [0, -10, 0], rotateZ: [-2, 2, -2] } : { y: 0, rotateZ: 0 }
        }
        ...
        >
          ...
          >
            ...
            <div className="relative overflow-hidden rounded-[1.3rem] bg-gradient-to-b from-[#141310] to-black px-3 pb-4 pt-6">
              ...
              <p className="relative mt-3 text-center text-[9px] uppercase tracking-wider text-white/50">
                Your Coach
              </p>
              <div className="relative mx-auto mt-2 h-12 w-12 overflow-hidden rounded-full border border-brand-yellow/50">
                <Image
                  src="/leanr-brand/coach-1.png"
                  alt="Your LeanR coach"
                  fill
                  unoptimized
                  sizes="48px"
              ...
              <div className="relative mt-3 flex items-center justify-center gap-1.5 text-[10px] font-semibold text-white/80">
                <span className="flex h-4 w-4 items-center justify-center rounded-full bg-brand-yellow text-[9px] text-black">
                  ✓
                </span>
                Session Confirmed
              </div>
              <p className="relative mt-1 text-center text-[9px] text-white/40">Today · 6:00 PM</p>
              <button className="relative mt-3 w-full rounded-full bg-brand-yellow py-1.5 text-[10px] font-bold text-black">
                Join Session
              </button>
            ...
            <div className="relative h-full overflow-hidden rounded-[1.3rem] bg-gradient-to-b from-[#141310] to-black px-3 pb-4 pt-6">
              ...
              <div className="relative mt-4 flex h-14 items-end justify-center gap-2">
                {[40, 65, 50, 90, 70, 30, 55].map((h, i) => (
                  <div
                    key={i}
                    className="w-2 rounded-full bg-brand-yellow/80"
                    style={{ height: `${h}%` }}
                  />
                ))}
              </div>
      ...
      <p className="mt-3 text-center text-[9px] uppercase tracking-wider text-white/30">
        Tap phone for weekly progress
      </p>
    </div>
  );
}
...
function CTAScene({ sectionRef }: { sectionRef: React.RefObject<HTMLElement | null> }) {
  ...
  return (
    <motion.div
      ...
      className="relative hidden h-full min-h-[22rem] items-center justify-center lg:flex"
...
export default function ReadyWhenYouAre() {
  ...
  return (
    <section
      id="book-demo"
      ref={sectionRef}
      className="relative min-h-screen overflow-hidden pt-24 pb-4 md:pt-28"
    >
      <FlipSection className="container-px">
        <Reveal>
          <GlassSectionPanel className="p-6 sm:p-9 noise">
            ...
            <div className="relative grid gap-8 lg:items-center lg:grid-cols-[1.1fr_0.9fr]">
              <div>
                ...
                <h2 className="text-display mt-4 text-4xl sm:text-5xl md:text-6xl leading-[0.9]">
                  READY
                  <br />
                  <span className="italic-skew text-brand-yellow">WHEN YOU ARE.</span>
                </h2>
                <p className="mt-4 max-w-md text-sm text-white/65 leading-relaxed">
                  Create your account and take the first step towards a better you — pick a plan or try a free demo session with a coach.
                </p>
                <div className="mt-5 flex flex-wrap gap-4 text-xs text-white/70">
                  {[
                    { label: "No Commitment", icon: ShieldCheckIcon },
                    { label: "Personalized Guidance", icon: CalendarIcon },
                    { label: "Expert Support", icon: HeadsetIcon },
                  ].map(({ label, icon: Icon }) => (
