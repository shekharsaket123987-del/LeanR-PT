...
function StatIcon({
  ...
  type: "people" | "star" | "target" | "shield";
  className?: string;
}) {
...
function TransformCard({
  ...
  return (
    <div style={{ perspective: 1200 }} className="h-full">
      {/* scroll entrance — center rises first, side cards follow */}
      <motion.div
        ...
        >
          ...
          >
            ...
            >
              <div
                className={cn(
                  "relative h-full overflow-hidden rounded-2xl p-2.5 transition-[border-color] duration-300",
                  isActive ? "glass-strong glow-yellow border-brand-yellow/50" : "glass"
                )}
              >
                {/* glass reflection — reacts subtly to the cursor on the active card */}
                {isActive && (
                  <motion.div
                ...
                <div className="relative aspect-[4/3] overflow-hidden rounded-lg">
                  <motion.div
                    style={{ x: isActive ? imgParallaxX : 0, y: isActive ? imgParallaxY : 0 }}
                    className="grid h-full grid-cols-2"
                  >
                    <div className="relative flex items-end justify-center bg-white/5 pb-2">
                      <span className="absolute left-2 top-1.5 text-[9px] font-semibold uppercase tracking-wider text-white/50">
                        Before
                      </span>
                    ...
                    >
                      <span className="absolute right-2 top-1.5 text-[9px] font-semibold uppercase tracking-wider text-brand-yellow">
                        After
                      </span>
        ...
        >
          ...
          >
            ...
            <div className="relative overflow-hidden rounded-[1.3rem] bg-gradient-to-b from-[#141310] to-black px-3 pb-4 pt-6">
              ...
              <button className="relative mt-2 w-full rounded-full bg-brand-yellow py-1.5 text-[10px] font-bold text-black">
                View Progress
              </button>
            </div>
          </div>
...
function StatsBar() {
  return (
    ...
    >
      <div className="glass grid grid-cols-2 gap-3 rounded-2xl px-4 py-3 sm:grid-cols-4 sm:gap-2 sm:px-6 sm:py-4">
        {STATS.map((s) => (
        ...
        <GlassSectionPanel className="p-6 noise sm:p-9">
          <Reveal className="mx-auto max-w-2xl text-center">
            ...
            <h2 className="text-display mt-3 text-3xl leading-[0.95] sm:text-4xl md:text-5xl">
              REAL PEOPLE. REAL <span className="italic-skew text-brand-yellow">RESULTS.</span>
            </h2>
            <p className="mt-2 text-sm text-white/60">
              Different journeys. Same goal. Healthier, stronger, happier.
            </p>
          </Reveal>
          ...
          <Reveal className="mt-10 flex flex-col items-center gap-4 text-center">
            <p className="text-display text-xl sm:text-2xl">
              Ready to start your <span className="italic-skew text-brand-yellow">transformation?</span>
            </p>
          </Reveal>
        </GlassSectionPanel>
      </FlipSection>
