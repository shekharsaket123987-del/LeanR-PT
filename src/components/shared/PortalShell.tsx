...
export default function PortalShell({
  ...
  return (
    <div className="min-h-screen bg-bg">
      {/* Mobile top bar */}
      <div className="sticky top-0 z-40 flex items-center justify-between border-b border-white/10 bg-bg-elevated/90 px-4 py-3 backdrop-blur-md lg:hidden">
        ...
        <button onClick={() => setMobileOpen(true)} className="rounded-lg p-2 text-white/70 hover:bg-white/5 hover:text-white">
      ...
      <div className="flex">
        {/* Sidebar */}
        <aside
          className={`fixed inset-y-0 left-0 z-50 w-72 transform border-r border-white/10 bg-bg-elevated transition-transform duration-200 lg:sticky lg:top-0 lg:z-0 lg:h-screen lg:translate-x-0 ${
          ...
          <div className="flex h-full flex-col">
            <div className="flex items-center justify-between px-6 py-6">
              <Link href="/" className="flex items-center gap-1.5">
                <span className="text-display text-2xl font-bold italic tracking-tight text-white">
                  LEANR
                </span>
                <span className="h-1.5 w-1.5 rounded-full bg-brand-yellow shadow-[0_0_10px_2px_rgba(245,217,10,0.8)]" />
              </Link>
              <button onClick={() => setMobileOpen(false)} className="rounded-lg p-1.5 text-white/60 hover:bg-white/5 lg:hidden">
                <X className="h-5 w-5" />
              </button>
            </div>
                ...
                return (
                  <Link
                    ...
                    className={`flex items-center gap-3 rounded-xl px-3.5 py-2.5 text-sm font-semibold transition-colors ${
                      active ? "glass-yellow text-white" : "text-white/55 hover:bg-white/5 hover:text-white"
                    }`}
                  >
