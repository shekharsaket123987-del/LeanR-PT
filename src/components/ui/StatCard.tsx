...
export default function StatCard({
  ...
  return (
    <Card dark={accent} className="p-5">
      ...
      <p className={cn("mt-4 text-display text-3xl font-bold italic leading-none text-white")}>{value}</p>
      <p className={cn("mt-1.5 text-sm font-medium", accent ? "text-white/60" : "text-white/50")}>{label}</p>
    </Card>
  );
}
