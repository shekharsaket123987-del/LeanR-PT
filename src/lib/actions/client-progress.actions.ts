...
import { createProgressLog, listProgressLogsForClient, ProgressLogInput, getMeasurementStatus } from "@/lib/services/progressLogs.service";
...
/** Lightweight status check for the compulsory-update modal and the
 * booking/join gates -- unlike getMyProgressAction, doesn't pull the full
 * log history, just the one date those call sites actually need. */
export async function getMyMeasurementStatusAction(): Promise<ActionResult<MeasurementReminderStatus>> {
  return runAction(async () => {
    const token = await requireToken();
    const client: any = await getMyClientProfile(token);
    return getMeasurementStatus(token, client.id);
  });
}
