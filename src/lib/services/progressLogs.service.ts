...
/** Single source of truth for the "measurements due" rule used across the
 * client compulsory-update gate, booking/join blocks, and the admin/coach
 * staleness badges -- null (never logged) counts as stale, same as an
 * old date. */
export function isMeasurementStale(lastLoggedAt: string | null): boolean {
  if (!lastLoggedAt) return true;
  const weekAgo = new Date();
  weekAgo.setDate(weekAgo.getDate() - 7);
  return new Date(lastLoggedAt) < weekAgo;
}

/** Bulk "most recent progress_logs.logged_at per client" lookup -- same
 * fetch-once-merge-in-JS pattern listClients()/listMyClients() already use
 * for subscriptions/slots, so the admin and coach client lists can show a
 * staleness badge without an N+1 query per row. */
export async function getLatestMeasurementDatesForClients(accessToken: string, clientIds: string[]): Promise<Map<string, string>> {
  ...
  for (const row of data ?? []) {
    if (!map.has(row.client_id)) map.set(row.client_id, row.logged_at);
  }
  return map;
}

/** Single-client convenience wrapper around getLatestMeasurementDatesForClients
 * -- used by the client-facing gate checks (demo booking, session booking,
 * the compulsory-update modal) where only one client's own status matters. */
export async function getMeasurementStatus(accessToken: string, clientId: string): Promise<{ lastLoggedAt: string | null; isStale: boolean }> {
  const dates = await getLatestMeasurementDatesForClients(accessToken, [clientId]);
  const lastLoggedAt = dates.get(clientId) ?? null;
  return { lastLoggedAt, isStale: isMeasurementStale(lastLoggedAt) };
}
